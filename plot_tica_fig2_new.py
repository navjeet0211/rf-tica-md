...
#for i in [1,2,5]:
    ...
    mplt.plot_free_energy(X[i][:,0], X[i][:,1], vmin=0, vmax=8, cmap='gnuplot2', ax=ax, levels=np.arange(0,7.1,0.5))
    ...
    mplt.plot_free_energy(Xsel[i][:,0], Xsel[i][:,1], vmin=0, vmax=9, cmap='jet', ax=ax)
    
    x_min, x_max, y_min, y_max, histogram = hist_raw_data(Xsel[i][:,[0,1]], np.log(y))
    histogram = histogram - np.min(histogram[~np.isnan(histogram)])
    plt.subplot(3,4,j+4)
    plt.imshow(histogram.T, origin='lower', extent=[x_min, x_max, y_min, y_max], aspect='auto', cmap=plt.cm.jet)
    plt.colorbar()
    '''
    j = j+1

plt.tight_layout()
plt.savefig('p450_supp_fig.png')
#plt.show()
